...
{
	public class SArticleWeb:IArticleWeb
	{
		#region 局部变量
		...
		#region 通用函数
		private SqlCommand GetCommand(MArticleWeb model)
		{
		...
		/// 实体信息
		/// </summary>
		private MArticleWeb GetModel(DataRow row)
		{
			MArticleWeb model = new MArticleWeb();
		...
		/// 增加
		/// </summary>
		public int Insert(MArticleWeb model, string ConnStr)
		{
			StringBuilder sql = new StringBuilder();
			sql.Append(" INSERT Web_ArticleWeb (");
		...
		{
			StringBuilder sql = new StringBuilder();
			sql.Append(" DELETE Web_ArticleWeb ");
			sql.Append(" WHERE 1=1" + strWhere);
			if (strWhere == "")
			{
				//清空列表
				sql = new StringBuilder();
				sql.Append(" TRUNCATE TABLE Web_ArticleWeb ");
			}
		...
		/// 修改
		/// </summary>
		public int Update(MArticleWeb model, string ConnStr)
		{
			StringBuilder sql = new StringBuilder();
			sql.Append(" INSERT Web_ArticleWeb");
			sql.Append(" SET");
		...
		public DataSet GetList(int pageSize, int pageIndex, string strWhere, string ConnStr)
		{
			...
			sql.Append(" b.WName, ");
			sql.Append(" c.Title ");
			sql.Append(" FROM Web_ArticleWeb a ");
		...
		/// 获取实体信息
		/// </summary>
		public MArticleWeb GetModel(string strWhere, string ConnStr)
		{
		...
		/// 获取实体列表
		/// </summary>
		public List<MArticleWeb> GetModelList(int pageSize, int pageIndex, string strWhere, string ConnStr)
		{
			List<MArticleWeb> modelList = new List<MArticleWeb>();

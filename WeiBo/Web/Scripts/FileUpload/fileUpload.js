...
var uploadTools = {
    ...
    "initWithLayout":function(opt){
        ...
        if(!opt.isHiddenUploadBt){
            ...
            btsStr += "<button class='iconfont icon-shangchuan'>上传</button>";
            btsStr += " </div>";
        }
        //清理按钮
        if(!opt.isHiddenCleanBt){
            btsStr += "<div class='cleanFileBt'>";
            btsStr += "<button class='iconfont icon-qingchu'>清除</button>";
            btsStr += " </div>";
        }
        btsStr += "</div>";
    ...
    "addFileList":function(fileList,opt){
        ...
        if(fileNumber+fileList.length > 1/*opt.maxFileNumber*/){
            alert("最多只能上传"+/*opt.maxFileNumber*/"1"+"个文件");
            return;
        }
